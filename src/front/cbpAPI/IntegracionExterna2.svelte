<script>
    ...
    async function loadGraph() {
        ...
        const resData_2 = await fetch("https://restcountries.eu/rest/v2/?fields=name;area");
        let MyData_2 = await resData_2.json();
        let countriesGroup = Array.from(MyData_2.map((d) => { return d.name; }));
        ...
        //Actives
        let area = Array.from(MyData_2.map((d) => { if (filteredcountry.includes(d.name)) return [d.name, parseInt(d.area)]; }))
        var filteredarea = area.filter(function (el) {
            return el != null;
        });
        ...
        console.log(reordena(filteredcountry, filteredarea));
        ...
        Highcharts.chart('container', {
            ...
            series: [{
                ...
                data: reordena(filteredcountry, filteredarea),
                tooltip: {
                    valuePrefix: '',
                    valueSuffix: ' '
                },
